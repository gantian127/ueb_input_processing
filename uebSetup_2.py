"""
This is the code to prepare the climate forcing:
- subset the data with bounding box
- concatenate the monthly/daily data as annual data with specific mon/date
"""
import os
...
for cYear in range(startYear,endYear):
    ...
    endDateTime=str(cYear+1)+'/10/01 0' #'/07/30 0'
    cbrfcPWS = watershedN+str(cYear+1)+'P.nc'
    cbrfcTWS = watershedN+str(cYear+1)+'T.nc'
    climateFunctions_2_ori.create_netCDF_from_multple_nc('mm', watershedN, cbrfcPWS, 'ogrid', leftX, topY, rightX, bottomY, startDateTime, endDateTime, 3, out_timeName ='time')
    climateFunctions_2_ori.create_netCDF_from_multple_nc('mm', watershedN, cbrfcTWS, 'otgrid', leftX, topY, rightX, bottomY, startDateTime, endDateTime, 3, out_timeName ='time')

print('CBRFC forcing done')
...
for cYear in range(startYear,endYear):
    ...
    climateFunctions_2_ori.subset_and_concatenate_netCDF_from_multple_nc_NLDAS('NLDAS_FORA0125_H.A_Monthly', watershedN, nldas2WS, leftX, topY, rightX, bottomY, startDateTime, endDateTime,
