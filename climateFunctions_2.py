"""
This is the latest version of processing model input
"""
try:
...
# for cbrfc forcing
def subset_multiple_netCDF_toTargetDir(wsName, out_prefix, output_dir,  leftX, topY, rightX, bottomY, startDateTime, endDateTime,
                                       in_Xcoord = 'x', in_Ycoord='y'):
    """
    Subsets and moves multiple nc to out dir
    ...
    endYear = datetime.strptime(endDateTime,"%Y/%m/%d %H").year
    for year in range(startYear, endYear+1):
        cmdString = "for i in mm_"+str(year)+"*.nc.gz; do gunzip -cdfq $i>${i//.gz} && do ncea -d "+in_Xcoord+","+str(leftX)+","+str(rightX)\
                    +" -d "+in_Ycoord+","+str(bottomY)+","+str(topY)+" -O ${i//.gz} ${i//.gz} && mv -f -t "+output_dir+" ${i//.gz}; done"
        #print(cmdString)
        callSubprocess(cmdString, 'subset nc and move')

# leftX=-112.0, topY=42.3, rightX=-111.0, bottomY=41.6, startYear=2009, endYear=2010
# for cbrfc forcing
def get_daily_minmax_netCDF_from_multple_nc(file_prefix, wsName, output_netcdf_min, output_netcdf_max, var_name, leftX, topY, rightX, bottomY,
                                  startDateTime, endDateTime, dT=3, out_timeName='time'):
    """
        Subsets and combines multiple netcdf files
        written for CBRFC daily min/max temp data sets
        for datasets that are commpressed with .gz
        """
    startYear = datetime.strptime(startDateTime, "%Y/%m/%d %H").year
    endYear = datetime.strptime(endDateTime, "%Y/%m/%d %H").year
    ...
    cmdString = 'for i in R_'+wsName+'_'+var_name+'*.nc; do ncra -v '+var_name+' -y min -d '+out_timeName+',,,'+str(hD)+','+str(hD)+' $i min_$i; done'
    callSubprocess(cmdString, "get daily minimum ")
    cmdString = 'for i in R_' + wsName + '_' + var_name + '*.nc; do ncra -v ' + var_name + ' -y max -d ' + out_timeName + ',,,' + str(hD) + ',' + str(hD) + ' $i max_$i; done'
    callSubprocess(cmdString, "get daily maximum ")

    cmdString = "ncrcat -H -h -O min_*.nc -o concat_" + output_netcdf_min  # -H don't append input file list -h don't append history
    callSubprocess(cmdString, "concatenate min netcdf files")
    cmdString = "ncrcat -H -h -O max_*.nc -o concat_" + output_netcdf_max  # -H don't append input file list -h don't append history
    callSubprocess(cmdString, "concatenate max netcdf files")

    cmdString = "ncea -4 -d " + out_timeName + "," + str(startDay) + "," + str(endDay) + " -H -h -O concat_" \
                + output_netcdf_min + " -o " + output_netcdf_min
    callSubprocess(cmdString, 'subset min netcdf in time')
    cmdString = "ncea -4 -d " + out_timeName + "," + str(startDay) + "," + str(endDay) + " -H -h -O concat_" \
                + output_netcdf_max + " -o " + output_netcdf_max
    callSubprocess(cmdString, 'subset max netcdf in time')

    dailyMin = 'dailymin_'+var_name
    dailyMax = 'dailymax_'+var_name

    # change var name
    ncOut = netCDF4.Dataset(output_netcdf_min, "r+", format='NETCDF4')
    ncOut.renameVariable(var_name, dailyMin)  # ncOut.createVariable(varNameVP,vardataType,('time','y','x',))
    ncOut.variables[dailyMin].setncattr('long_name', 'Daily minimum temperature')
    ncOut.close()
    ncOut = netCDF4.Dataset(output_netcdf_max, "r+", format='NETCDF4')
    ncOut.renameVariable(var_name, dailyMax)  # ncOut.createVariable(varNameVP,vardataType,('time','y','x',))
    ncOut.variables[dailyMax].setncattr('long_name', 'Daily minimum temperature')
    ncOut.close()

    # delete intermediate files
    cmdString = "rm -f R_*.nc "
    callSubprocess(cmdString, "delete intermediate files")
    cmdString = "rm -f min_*.nc "
    callSubprocess(cmdString, "delete intermediate files")
    cmdString = "rm -f max_*.nc "
    callSubprocess(cmdString, "delete intermediate files")
    cmdString = "rm -f concat_*.nc "
    callSubprocess(cmdString, "delete intermediate files")
...
def compute_average_windSpeed(input_netcdfU, varNameU, input_netcdfV, varNameV, output_netcdfW, varNameW ):            #output_netcdfVP, varNameVP,
    ...
    #delete old variable
    cmdString = "ncks -4 -C -h -O "+input_netcdfU+" "+output_netcdfW
    callSubprocess(cmdString, 'copy nc file')
    ...
    """
    # Copy attributes
    varAtts = ncInU.variables[varNameU].ncattrs()
    attDict = dict.fromkeys(varAtts)
    for attName in varAtts:
        attDict[attName] = getattr(ncInU.variables[varNameU], attName)
        if attName == 'long_name':
            attDict[attName] = 'Average wind speed at height 10 m'
    """

    ncOut = netCDF4.Dataset(output_netcdfW,"r+", format='NETCDF4')
    ncOut.renameVariable(varNameU,varNameW)      #ncOut.createVariable(varNameW,vardataType,('time','y','x',))
    ncOut.variables[varNameW].setncattr('long_name','Average wind speed at height 10 m')
    ...
    ncInV.close()
    ncOut.close()
    cmdString = "ncks -4 -C -h -O -x -v " + varNameU + " " + output_netcdfW + " " + output_netcdfW
...
def compute_vaporPressure_from_SpecficHumidity(input_netcdf_q, varNameq, input_netcdfP, varNameP,  output_netcdfVP, varNameVP ):            #output_netcdfVP, varNameVP,
    ...
    #delete old variable
    cmdString = "ncks -4 -C -h -O "+input_netcdf_q+" "+output_netcdfVP
    callSubprocess(cmdString, 'copy nc')
    ...
    ncOut.renameVariable(varNameq, varNameVP)   #ncOut.createVariable(varNameVP,vardataType,('time','y','x',))
    ncOut.variables[varNameVP].setncattr('long_name', 'Vapor pressure')
    ncOut.variables[varNameVP].setncattr('units', 'Pa')
    #varin = numpy.zeros((len(yin),len(xin)),dtype=vardataType)
    #varout = numpy.zeros((len(yout),len(xout)),dtype=vardataType)
    ...
    ncInq.close()
    ncOut.close()
    cmdString = "ncks -4 -C -h -O -x -v " + varNameq + " " + output_netcdfVP + " " + output_netcdfVP
    callSubprocess(cmdString, 'delete old/reference variable')
    #delete temp netcdf file
    #os.remove(temp_netcdf)
